...
EDR_TELEMETRY_URL = os.getenv("EDR_TELEMETRY_URL")
EDR_TELEMETRY_README_URL = os.getenv("EDR_TELEMETRY_README_URL")
TECHNIQUE_REPORT_URL = os.getenv("TECHNIQUE_REPORT_URL")
ADVERSARY_REPORT_URL = os.getenv("ADVERSARY_REPORT_URL")
...
def warn_missing_required_env():
    missing = [name for name, value in _REQUIRED_VARS.items() if not value]
    if missing:
        print(
            f"[WARNING] The following required environment variables are not set: "
            f"{', '.join(missing)}. "
            "Copy .env.example to .env and fill in the values before running agents.",
            file=sys.stderr,
        )
