...
def _configured_url(name):
    value = getattr(utils.constants, name, None)
    return value or f"<set {name} in .env>"
...
actions = {
    ...
    "SUMMARIZE_RECENT_CISA_VULNS": [
        {
            "message": f"""Run a single Shell command to download (using curl -sS) {_configured_url("CISA_KEV_URL")},
    ...
    "IDENTIFY_EDR_BYPASS_TECHNIQUES": [
        {
            "message": f"Identify the EDR telemetry gaps for Elastic using {_configured_url('EDR_TELEMETRY_URL')}",
            "summary_method": "last_msg",
    ...
    "DETECT_EDR": [
        {
            "message": f"Download {_configured_url('EDR_TELEMETRY_README_URL')}",
            "summary_method": "last_msg",
    ...
    "TTP_REPORT_TO_TECHNIQUES": [
        {
            "message": f"Download the HTML report at {_configured_url('TECHNIQUE_REPORT_URL')}",
            "summary_method": "last_msg",
    ...
    "TTP_REPORT_TO_ADVERSARY_PROFILE": [
        {
            "message": f"Download the HTML report at {_configured_url('ADVERSARY_REPORT_URL')} and extract the MITRE techniques.",
            "summary_method": "last_msg",
            "carryover": "Replace this placeholder with all the MITRE techniques extracted from the downloaded report.",
            "agent": "internet_agent",
        },
